...
export function App() {
  ...
  const [mainImageTransitioning, setMainImgTransitioning] =
  ...
  useEffect(() => {
    setImageDataArray(
      isNil(state.shuffledImageData) ? state.imageData : state.shuffledImageData
    );

    setHideMainImage(lightboxMode);

    if (imageDataArray && imageDataArray.length > 0) {
      setMainImgSrc(imageDataArray[state.activeIndex ?? 0].imgSrc as string);
      setVideoSrc(
        isNil(imageDataArray[state.activeIndex ?? 0]?.videoSrc)
          ? ""
          : (imageDataArray[state.activeIndex ?? 0]?.videoSrc as string)
      );
    }
  }, [
    hideMainImage,
            ...
            ? [
                {
                  className: "image",
                  imgSrc: dataUrl,
                  imgFilters: [state.filters[state.filterIndex]],
                  videoSrc: null,
                  navIndex: null,
    ...
    <div className="app-container">
      <>
        <header className={classNames(isFlyoutVisible && "visible")}>
          ...
          >
            <i className="fa-solid fa-arrow-up" style={{ color: "#141414" }} />
          </span>
        </header>
        <TransitionGroup>
          <CSSTransition
          ...
          >
            <main ref={imgeRef} className="image-wrapper">
              ...
              {state.collageImageData && !state.hideCollageDisplay ? (
                ...
                <>
                  ...
                  base64Mime(videoSrc) === "video/mp4" ? (
                    ...
                    <Draggable>
                      <span>
                        {isEmpty(state.imageData) || hideMainImage ? (
                          <></>
                        ) : (
                          <Image
                            classArray={[
                            ...
                            filters={
                              state.collageImageFilters
                                ? state.collageImageFilters[state.filterIndex]
                                : state.filters[0]
                            }
                            imgSrc={
                            ...
                            imgId={
                              state.collageImageData
                                ? `${
                                    state.collageImageData[state.filterIndex]
                                      .imgId
                                  }`
                                : state.imageData
        ...
        >
          <div
            ...
            className={classNames(
              isInitialRender ? "flyout-transition-enter-done" : "",
              "flyout-wrapper"
            )}
          >
            <Flyout
              isCreateCollageCheckboxChecked={isCreateCollageCheckboxChecked}
